...
namespace OgreAL {
  ...
  Ogre::Real Sound::getGainScale() const {
    return mGainScale;
  }

  void Sound::setGainScale(Ogre::Real gainScale) {
    // Set the new gain scale, clamping to [0,1].
    mGainScale = Ogre::Math::Clamp(gainScale, Ogre::Real(0.0), Ogre::Real(1.0));

    // Set the actual gain based on the current gain scale.
    setGain(mGain);
  }

  void Sound::setGainValues(Ogre::Real maxGain, Ogre::Real minGain, 
			    Ogre::Real gain, Ogre::Real gainScale) {
