...
//-----------------------------------------------------------------------------
///
void WindBatchPage::init(PagedGeometry *geom, const Ogre::Any &data)
{
   ...
   m_bShadersSupported = caps->hasCapability(Ogre::RSC_VERTEX_PROGRAM) ? true : false;    // <-- DELETE THIS
		...
		for (unsigned short i = 0; i < subBatch->m_pVertexData->vertexDeclaration->getElementCount(); ++i)
      {
		...
		if (Ogre::HighLevelGpuProgramManager::getSingleton().getByName(vertexProgName).isNull())
		{
			...
			if(!shaderLanguage.compare("hlsl") || !shaderLanguage.compare("cg"))
			{
				...
				{
					const Ogre::VertexElement *el = subBatch->m_pVertexData->vertexDeclaration->getElement(j);
					if (el->getSemantic() == Ogre::VES_TEXTURE_COORDINATES) 
					{
						++ texCoordCount;
					}
				...
				for (unsigned short i = 0; i < subBatch->m_pVertexData->vertexDeclaration->getElementCount(); ++i)
				{
					...
					if (el->getSemantic() == Ogre::VES_TEXTURE_COORDINATES)
					{
						...
						{
							vertexProgSource += 
								"	float4 params 	: TEXCOORD" + Ogre::StringConverter::toString(texCoordCount-2) + ", \n";
						}
						else
						{
							...
							{
								vertexProgSource += 
									"	float4 originPos 	: TEXCOORD" + Ogre::StringConverter::toString(texCoordCount-1) + ", \n";
							}
							else
							{
								Ogre::String uvType = "";
				...
				{
					vertexProgSource += 
						"	oUV" + Ogre::StringConverter::toString(i) + " = iUV" + Ogre::StringConverter::toString(i) + "; \n";
				}
			...
			if(!shaderLanguage.compare("glsl"))
			{
				...
				for (unsigned short i = 0; i < subBatch->m_pVertexData->vertexDeclaration->getElementCount(); ++i)
				{
					...
					if (el->getSemantic() == Ogre::VES_TEXTURE_COORDINATES)
					{
						...
						{
							vertexProgSource += 
								"	vec4 params = gl_MultiTexCoord" + Ogre::StringConverter::toString(texCoordCount-2) + "; \n";
						}
						else
						{
							if (el->getIndex() == texCoordCount - 1)
							{
								vertexProgSource += 
									"	vec4 originPos = gl_MultiTexCoord" + Ogre::StringConverter::toString(texCoordCount-1) + "; \n";
							}
							else
							{
								vertexProgSource += 
				...
				for (unsigned short p = 0; p < tech->getNumPasses(); ++p){
					...
					try{
						...
						if (lightingEnabled) {
							...
							//params->setNamedAutoConstant("matAmbient", Ogre::GpuProgramParameters::ACT_SURFACE_AMBIENT_COLOUR);
						}
