...
//-----------------------------------------------------------------------------
/// Constructor
StaticBillboardSet::StaticBillboardSet(Ogre::SceneManager *mgr, Ogre::SceneNode *rootSceneNode, BillboardMethod method) :
mVisible                (true),
mFadeEnabled            (false),
...
mFadeInvisibleDist      (0.f)
{
   ...
   if (mRenderMethod == BB_METHOD_ACCELERATED)
   {
      ...
      {
         const Ogre::String &renderName = Ogre::Root::getSingleton().getRenderSystem()->getName();
         ...
         Ogre::HighLevelGpuProgramPtr vertexShader = Ogre::HighLevelGpuProgramManager::getSingleton().getByName("Sprite_vp");
         assert(vertexShader.isNull() && "Sprite_vp already exist");

         vertexShader = Ogre::HighLevelGpuProgramManager::getSingleton().createProgram(
            "Sprite_vp", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, shaderLanguage, Ogre::GPT_VERTEX_PROGRAM);
         ...
         Ogre::HighLevelGpuProgramPtr vertexShader2 = Ogre::HighLevelGpuProgramManager::getSingleton().getByName("SpriteFade_vp");
         assert(vertexShader2.isNull() && "SpriteFade_vp already exist");
         vertexShader2 = Ogre::HighLevelGpuProgramManager::getSingleton().createProgram("SpriteFade_vp",
            Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, shaderLanguage, Ogre::GPT_VERTEX_PROGRAM);
...
void StaticBillboardSet::clear()
{
   if (mRenderMethod == BB_METHOD_ACCELERATED)
   {
      ...
      if (mpEntity)
      {
         ...
         //Delete mesh
         Ogre::String meshName(mPtrMesh->getName());
         mPtrMesh.setNull();
...
void StaticBillboardSet::build()
{
   if (mRenderMethod == BB_METHOD_ACCELERATED)
   {
      ...
      mPtrMesh = Ogre::MeshManager::getSingleton().createManual(getUniqueID("SBSmesh"), Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
      ...
      Ogre::VertexDeclaration* dcl = pSubMesh->vertexData->vertexDeclaration;
      size_t offset = 0;
      ...
      Ogre::HardwareVertexBufferSharedPtr vbuf = Ogre::HardwareBufferManager::getSingleton().createVertexBuffer(
         offset, pSubMesh->vertexData->vertexCount, Ogre::HardwareBuffer::HBU_STATIC_WRITE_ONLY, false);
      float* pReal = static_cast<float*>(vbuf->lock(Ogre::HardwareBuffer::HBL_DISCARD));

      float minX = (float)Ogre::Math::POS_INFINITY, minY = (float)Ogre::Math::POS_INFINITY, minZ = (float)Ogre::Math::POS_INFINITY;
      float maxX = (float)Ogre::Math::NEG_INFINITY, maxY = (float)Ogre::Math::NEG_INFINITY, maxZ = (float)Ogre::Math::NEG_INFINITY;
      ...
      for (size_t ibb = 0; ibb < billboardCount; ++ibb)
      {
         ...
         *pReal++ = 0.0f;
         ...
         // color
         *(reinterpret_cast< Ogre::uint32* >(pReal++)) = bb->color;
         // uv
      ...
      // Populate index buffer
      {
         ...
         ptrIndBuf = Ogre::HardwareBufferManager::getSingleton().createIndexBuffer(Ogre::HardwareIndexBuffer::IT_16BIT,
            pSubMesh->indexData->indexCount, Ogre::HardwareBuffer::HBU_STATIC_WRITE_ONLY, false);

         Ogre::uint16 *pIBuf = static_cast < Ogre::uint16* > (ptrIndBuf->lock(Ogre::HardwareBuffer::HBL_DISCARD));
      ...
      // Finish up mesh
      {
         Ogre::AxisAlignedBox bounds(minX, minY, minZ, maxX, maxY, maxZ);
         ...
         // Loading mesh
         Ogre::LoggingLevel logLev = Ogre::LogManager::getSingleton().getDefaultLog()->getLogDetail();
         Ogre::LogManager::getSingleton().setLogDetail(Ogre::LL_LOW);
         mPtrMesh->load();
         Ogre::LogManager::getSingleton().setLogDetail(logLev);
      }
...
//-----------------------------------------------------------------------------
///
void StaticBillboardSet::setMaterial(const Ogre::String &materialName, const Ogre::String &resourceGroup)
{
   ...
   if (mRenderMethod == BB_METHOD_ACCELERATED)
   {
      ...
      mPtrMaterial = Ogre::MaterialManager::getSingleton().getByName(materialName, resourceGroup);
...
//-----------------------------------------------------------------------------
///
void StaticBillboardSet::setFade(bool enabled, Ogre::Real visibleDist, Ogre::Real invisibleDist)
{
   if (mRenderMethod == BB_METHOD_ACCELERATED)
   {
      if (enabled)
      {
         if (mPtrMaterial.isNull())
            OGRE_EXCEPT(Ogre::Exception::ERR_ITEM_NOT_FOUND, "Billboard fading cannot be enabled without a material applied first", "StaticBillboardSet::setFade()");
...
//-----------------------------------------------------------------------------
///
Ogre::MaterialPtr StaticBillboardSet::getFadeMaterial(const Ogre::MaterialPtr &protoMaterial,
                                                Ogre::Real visibleDist_, Ogre::Real invisibleDist_)
{
   assert(!protoMaterial.isNull());

   Ogre::StringUtil::StrStreamType materialSignature;
   ...
   {
      Ogre::MaterialPtr fadeMaterial = protoMaterial->clone(getUniqueID("ImpostorFade"));

      bool isglsl = Ogre::Root::getSingleton().getRenderSystem()->getName() == "OpenGL Rendering Subsystem" ? true : false;

      //And apply the fade shader
      for (unsigned short t = 0; t < fadeMaterial->getNumTechniques(); ++t)
      {
         Ogre::Technique *tech = fadeMaterial->getTechnique(t);
         for (unsigned short p = 0; p < tech->getNumPasses(); ++p)
         {
            ...
            params->setNamedAutoConstant(uScroll, Ogre::GpuProgramParameters::ACT_CUSTOM);
            params->setNamedAutoConstant(vScroll, Ogre::GpuProgramParameters::ACT_CUSTOM);
            params->setNamedAutoConstant(preRotatedQuad0, Ogre::GpuProgramParameters::ACT_CUSTOM);
            params->setNamedAutoConstant(preRotatedQuad1, Ogre::GpuProgramParameters::ACT_CUSTOM);
            params->setNamedAutoConstant(preRotatedQuad2, Ogre::GpuProgramParameters::ACT_CUSTOM);
            params->setNamedAutoConstant(preRotatedQuad3, Ogre::GpuProgramParameters::ACT_CUSTOM);

            params->setNamedAutoConstant(camPos, Ogre::GpuProgramParameters::ACT_CAMERA_POSITION_OBJECT_SPACE);
            params->setNamedAutoConstant(fadeGap, Ogre::GpuProgramParameters::ACT_CUSTOM);
            params->setNamedAutoConstant(invisibleDist, Ogre::GpuProgramParameters::ACT_CUSTOM);
      ...
      s_mapFadedMaterial.insert(std::pair<Ogre::String, Ogre::MaterialPtr>(materialSignature.str(), fadeMaterial));
...
void StaticBillboardSet::updateAll(const Ogre::Vector3 &cameraDirection)
{
   ...
   Ogre::Vector3 forward = cameraDirection;
   Ogre::Vector3 vRight = forward.crossProduct(Ogre::Vector3::UNIT_Y);
   Ogre::Vector3 vUp = forward.crossProduct(vRight);
   vRight.normalise();
   vUp.normalise();
   ...
   Ogre::Vector3 vPoint0 = (-vRight + vUp);
   Ogre::Vector3 vPoint1 = ( vRight + vUp);
   Ogre::Vector3 vPoint2 = (-vRight - vUp);
   Ogre::Vector3 vPoint3 = ( vRight - vUp);
   ...
   const Ogre::GpuConstantDefinition *pGPU_ConstDef_preRotatedQuad0 = 0,
   ...
   while (i1 != iend)
   {
      ...
      const float *pQuad = i1->second->getOrigin() == Ogre::BBO_CENTER ? preRotatedQuad_BBO_CENTER : preRotatedQuad_BBO_BOTTOM_CENTER;
      ...
      {
         const Ogre::GpuConstantDefinition *def = params->_findNamedConstantDefinition(preRotatedQuad0, true);
...
{
   assert((origin == Ogre::BBO_CENTER || origin == Ogre::BBO_BOTTOM_CENTER) && "Invalid origin - only BBO_CENTER and BBO_BOTTOM_CENTER is supported");
   mBBOrigin = origin;
   if (mRenderMethod != BB_METHOD_ACCELERATED)
...
void SBMaterialRef::addMaterialRef(const Ogre::MaterialPtr &matP, Ogre::BillboardOrigin o)
{
   Ogre::Material *mat = matP.getPointer();
...
void SBMaterialRef::removeMaterialRef(const Ogre::MaterialPtr &matP)
{
   Ogre::Material *mat = matP.getPointer();
...
SBMaterialRef::SBMaterialRef(Ogre::Material *mat, Ogre::BillboardOrigin o)
{
   material = mat;
   origin = o;
   refCount = 1;
