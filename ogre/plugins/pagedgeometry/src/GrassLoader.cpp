...
Ogre::Mesh *GrassLoader::generateGrass_QUAD(PageInfo &page, GrassLayer *layer, const float *grassPositions, unsigned int grassCount)
{
	...
	unsigned int maxUInt16 = std::numeric_limits<Ogre::uint16>::max();
	if(grassCount > maxUInt16)
	{
		Ogre::LogManager::getSingleton().logMessage("grass count overflow: you tried to use more than " + Ogre::StringConverter::toString(maxUInt16) + " (thats the maximum) grass meshes for one page");
		return 0;
	}
	if(quadCount > maxUInt16)
	{
		Ogre::LogManager::getSingleton().logMessage("quad count overflow: you tried to use more than " + Ogre::StringConverter::toString(maxUInt16) + " (thats the maximum) grass meshes for one page");
		return 0;
	}
	...
	Ogre::Real minY = Ogre::Math::POS_INFINITY, maxY = Ogre::Math::NEG_INFINITY;
	...
	for (Ogre::uint16 i = 0; i < grassCount; ++i)
	{
		...
		Ogre::Real xTrans = Ogre::Math::Cos(angle) * halfScaleX;
		Ogre::Real zTrans = Ogre::Math::Sin(angle) * halfScaleX;
		...
		if (heightFunction)
      {
			...
			if (layer->getMaxSlope() < (Ogre::Math::Abs(y1 - y2) / (halfScaleX * 2))) {
				//Degenerate the face
				x2 = x1;
				y2 = y1;
				z2 = z1;
...
Ogre::Mesh *GrassLoader::generateGrass_CROSSQUADS(PageInfo &page, GrassLayer *layer, const float *grassPositions, unsigned int grassCount)
{
	//Calculate the number of quads to be added
	unsigned int quadCount;
...
Ogre::Mesh *GrassLoader::generateGrass_SPRITE(PageInfo &page, GrassLayer *layer, const float *grassPositions, unsigned int grassCount)
{
	//Calculate the number of quads to be added
	unsigned int quadCount;
	quadCount = grassCount;
	...
	for (Ogre::uint16 i = 0; i < grassCount; ++i)
	{
      ...
      float y = heightFunction ? (float)heightFunction(x, z, heightFunctionUserData) : 0.f;
		...
		//Get the color at the grass position
      Ogre::uint32 color = layer->colorMap ? layer->colorMap->getColorAt(x, z, layer->mapBounds) : 0xFFFFFFFF;
...
void GrassLayer::setMaterialName(const Ogre::String &matName)
{
	if (material.isNull() || matName != material->getName()){
		...
		if (material.isNull())
			OGRE_EXCEPT(Ogre::Exception::ERR_INVALIDPARAMS, "The specified grass material does not exist", "GrassLayer::setMaterialName()");
		shaderNeedsUpdate = true;
	}
}
...
void GrassLayer::setDensityMap(const Ogre::String &mapFile, MapChannel channel)
{
	if (densityMap){
		densityMap->unload();
		densityMap = NULL;
...
void GrassLayer::setDensityMap(Ogre::TexturePtr map, MapChannel channel)
{
	if (densityMap){
		densityMap->unload();
		densityMap = NULL;
	...
	if (!minY && !maxY)
   {
		...
		for (unsigned int i = 0; i < grassCount; ++i)
      {
         ...
         else if (x >= mapBounds.left && x <= mapBounds.right && z >= mapBounds.top && z <= mapBounds.bottom)
         {
			...
			*posPtr++ = parent->rTable->getRangeRandom(0, (float)Ogre::Math::TWO_PI);
		}
	}
   else
   {
		//Height range
		Ogre::Real min, max;
		if (minY) min = minY; else min = Ogre::Math::NEG_INFINITY;
		if (maxY) max = maxY; else max = Ogre::Math::POS_INFINITY;
		...
		for (unsigned int i = 0; i < grassCount; ++i){
			...
			if (y >= min && y <= max){
				...
				if (!colorMap){
					...
					*posPtr++ = z;
					*posPtr++ = parent->rTable->getUnitRandom();
					*posPtr++ = parent->rTable->getRangeRandom(0, (float)Ogre::Math::PI);
				} else if (x >= mapBounds.left && x <= mapBounds.right && z >= mapBounds.top && z <= mapBounds.bottom){
					*posPtr++ = x;
					*posPtr++ = z;
					*posPtr++ = parent->rTable->getUnitRandom();
...
void GrassLayer::setColorMap(const Ogre::String &mapFile, MapChannel channel)
{
	if (colorMap){
		colorMap->unload();
		colorMap = NULL;
...
void GrassLayer::setColorMap(Ogre::TexturePtr map, MapChannel channel)
{
	if (colorMap){
		colorMap->unload();
		colorMap = NULL;
...
void GrassLayer::_updateShaders()
{
	if (shaderNeedsUpdate){
		...
		if (caps->hasCapability(Ogre::RSC_VERTEX_PROGRAM) && geom->getShadersEnabled())
		{
			...
			tmpName << "vp";
			const Ogre::String vsName = tmpName.str();

			//Generate a string ID that identifies the material combined with the vertex shader
			const Ogre::String matName = material->getName() + "_" + vsName;

			//Check if the desired material already exists (if not, create it)
			Ogre::MaterialPtr tmpMat = Ogre::MaterialManager::getSingleton().getByName(matName);
			if (tmpMat.isNull())
			{
				...
				Ogre::HighLevelGpuProgramPtr vertexShader = Ogre::HighLevelGpuProgramManager::getSingleton().getByName(vsName);
				...
				Ogre::Pass *pass = tmpMat->getTechnique(0)->getPass(0);
				...
				params->setNamedAutoConstant("fadeRange", Ogre::GpuProgramParameters::ACT_CUSTOM, 1);

				if (animate){
					params->setNamedAutoConstant("time", Ogre::GpuProgramParameters::ACT_CUSTOM, 1);
					params->setNamedAutoConstant("frequency", Ogre::GpuProgramParameters::ACT_CUSTOM, 1);
					params->setNamedAutoConstant("direction", Ogre::GpuProgramParameters::ACT_CUSTOM, 4);
				}

				if (lighting){
					params->setNamedAutoConstant("objSpaceLight", Ogre::GpuProgramParameters::ACT_LIGHT_POSITION_OBJECT_SPACE);
					params->setNamedAutoConstant("lightDiffuse", Ogre::GpuProgramParameters::ACT_DERIVED_LIGHT_DIFFUSE_COLOUR);
					params->setNamedAutoConstant("lightAmbient", Ogre::GpuProgramParameters::ACT_DERIVED_AMBIENT_LIGHT_COLOUR);
				}

				if (fadeTechnique == FADETECH_GROW || fadeTechnique == FADETECH_ALPHAGROW){
					params->setNamedAutoConstant("grassHeight", Ogre::GpuProgramParameters::ACT_CUSTOM, 1);
...
void GrassPage::addEntity(Ogre::Entity *entity, const Ogre::Vector3 &position, const Ogre::Quaternion &rotation, const Ogre::Vector3 &scale, const Ogre::ColourValue &color)
{
...
{
	std::list<Ogre::SceneNode*>::iterator i;
	for (i = nodeList.begin(); i != nodeList.end(); ++i)
	{
		...
		{
			Ogre::Entity *ent = static_cast<Ogre::Entity*>(node->getAttachedObject(j));
			if(!ent) continue;
			// remove the mesh
