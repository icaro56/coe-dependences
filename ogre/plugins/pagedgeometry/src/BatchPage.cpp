...
//-----------------------------------------------------------------------------
///
void BatchPage::init(PagedGeometry *geom_, const Ogre::Any &data)
{
...
#ifdef _DEBUG
	if (datacast < 0)
		OGRE_EXCEPT(Ogre::Exception::ERR_INVALIDPARAMS,"Data of BatchPage must be a positive integer. It representing the LOD level this detail level stores.","BatchPage::BatchPage");
#endif
   ...
   m_nLODLevel    = datacast;
   ...
   else
	{
      ...
      m_bShadersSupported = caps->hasCapability(Ogre::RSC_VERTEX_PROGRAM) ? true : false;
	}
...
//-----------------------------------------------------------------------------
///
void BatchPage::addEntity(Ogre::Entity *ent, const Ogre::Vector3 &position, const Ogre::Quaternion &rotation,
...
void BatchPage::build()
{
	...
	while (it.hasMoreElements())
   {
		...
		const Ogre::MaterialPtr &ptrMat = subBatch->getMaterial();
      ...
      {
			Ogre::Technique *tech = ptrMat->getTechnique(t);
...
//-----------------------------------------------------------------------------
///
void BatchPage::setFade(bool enabled, Ogre::Real visibleDist, Ogre::Real invisibleDist)
{
	...
	if (m_bFadeEnabled != enabled)
	{
		m_bFadeEnabled = enabled;
         ...
         m_pBatchGeom->setRenderQueueGroup(m_pPagedGeom ? m_pPagedGeom->getRenderQueue() : Ogre::RENDER_QUEUE_6);
      else
         //Opaque batches should render in the normal render queue
         m_pBatchGeom->setRenderQueueGroup(Ogre::RENDER_QUEUE_MAIN);
...
void BatchPage::_updateShaders()
{
   ...
   {
		BatchedGeometry::SubBatch *subBatch = it.getNext();
		const Ogre::MaterialPtr &ptrMat = m_vecUnfadedMaterials[i++];
		...
		for (unsigned short t = 0, tCnt = ptrMat->getNumTechniques(); t < tCnt; ++t)
      {
		...
		if (lightingEnabled)
			tmpName << "lit_";
		if (subBatch->m_pVertexData->vertexDeclaration->findElementBySemantic(Ogre::VES_DIFFUSE) != NULL)
			tmpName << "clr_";

		for (size_t i = 0, iCnt = subBatch->m_pVertexData->vertexDeclaration->getElementCount(); i < iCnt; ++i)
		{
			const Ogre::VertexElement *el = subBatch->m_pVertexData->vertexDeclaration->getElement(i);
			if (el->getSemantic() == Ogre::VES_TEXTURE_COORDINATES)
         {
				Ogre::String uvType;
            switch (el->getType())
            {
            case Ogre::VET_FLOAT1: uvType = "1"; break;
            case Ogre::VET_FLOAT2: uvType = "2"; break;
            case Ogre::VET_FLOAT3: uvType = "3"; break;
            case Ogre::VET_FLOAT4: uvType = "4"; break;
            }
            tmpName << uvType << '_';
			}
		}
		...
		const Ogre::String vertexProgName = tmpName.str();
		...
		if (Ogre::HighLevelGpuProgramManager::getSingleton().getByName(vertexProgName).isNull())
		{
			Ogre::Pass *pass = ptrMat->getTechnique(0)->getPass(0);
			...
			if(!shaderLanguage.compare("hlsl") || !shaderLanguage.compare("cg"))
			{
				...
				if (subBatch->m_pVertexData->vertexDeclaration->findElementBySemantic(Ogre::VES_DIFFUSE) != NULL) vertexProgSource +=
					"	float4 iColor    : COLOR, \n";
				...
				for (unsigned short i = 0; i < subBatch->m_pVertexData->vertexDeclaration->getElementCount(); ++i)
            {
					...
					if (el->getSemantic() == Ogre::VES_TEXTURE_COORDINATES)
               {
                  ...
                  switch (el->getType())
                  {
                  case Ogre::VET_FLOAT1: uvType = "float"; break;
                  case Ogre::VET_FLOAT2: uvType = "float2"; break;
                  case Ogre::VET_FLOAT3: uvType = "float3"; break;
                  case Ogre::VET_FLOAT4: uvType = "float4"; break;
                  }

						vertexProgSource +=
						"	" + uvType + " iUV" + Ogre::StringConverter::toString(texNum) + "			: TEXCOORD" + Ogre::StringConverter::toString(texNum) + ",	\n"
						"	out " + uvType + " oUV" + Ogre::StringConverter::toString(texNum) + "		: TEXCOORD" + Ogre::StringConverter::toString(texNum) + ",	\n";
				...
				for (size_t i = 0, iCnt = subBatch->m_pVertexData->vertexDeclaration->getElementCount(); i < iCnt; ++i)
            {
               ...
               {
						vertexProgSource +=
						"	oUV" + Ogre::StringConverter::toString(texNum) + " = iUV" + Ogre::StringConverter::toString(texNum) + ";	\n";
						++texNum;
					}
				}
			...
			if(!shaderLanguage.compare("glsl"))
			{
				...
				for (size_t i = 0, iCnt = subBatch->m_pVertexData->vertexDeclaration->getElementCount(); i < iCnt; ++i)
				{
					...
					{
						vertexProgSource +=
						"   gl_TexCoord[" + Ogre::StringConverter::toString(texNum) + "] = gl_MultiTexCoord" + Ogre::StringConverter::toString(texNum) + ";	\n";
						++texNum;
					}
				}
			...
			Ogre::HighLevelGpuProgramPtr vertexShader = Ogre::HighLevelGpuProgramManager::getSingleton().createProgram(
				vertexProgName, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, shaderLanguage, Ogre::GPT_VERTEX_PROGRAM);
		...
		if (m_bFadeEnabled)
      {
		...
		Ogre::MaterialPtr generatedMaterial = Ogre::MaterialManager::getSingleton().getByName(materialSignature.str());
		if (generatedMaterial.isNull())
      {
         ...
         {
				Ogre::Technique *tech = generatedMaterial->getTechnique(t);
				for (unsigned short p = 0, pCnt = tech->getNumPasses(); p < pCnt; ++p)
            {
					Ogre::Pass *pass = tech->getPass(p);
