...
//-------------------------------------------------------------------------------------
PagedGeometry::PagedGeometry(Ogre::Camera* cam, const Ogre::Real pageSize, Ogre::RenderQueueGroupID queue) :
m_nRenderQueue(queue)
{
   ...
   else
   {
		...
		sceneMgr = NULL;
		rootNode = NULL;
		oldCamPos = Ogre::Vector3::ZERO;
	}
	lastSceneCam = NULL;
	lastOldCamPos = Ogre::Vector3::ZERO;
...
void PagedGeometry::setCamera(Ogre::Camera *cam)
{
	if (cam == NULL){
		...
		if (sceneMgr && cam->getSceneManager() != sceneMgr)
			OGRE_EXCEPT(Ogre::Exception::ERR_INVALIDPARAMS, "The specified camera is from the wrong SceneManager", "PagedGeometry::setCamera()");
...
Ogre::Vector3 PagedGeometry::_convertToLocal(const Ogre::Vector3 &globalVec) const
{
	return globalVec;
}
#endif

void PagedGeometry::setPageSize(Ogre::Real size)
{
...
void PagedGeometry::setBounds(TBounds bounds)
{
	...
	if (!Ogre::Math::RealEqual(bounds.width(), bounds.height(), 0.01f))
		OGRE_EXCEPT(Ogre::Exception::ERR_INVALIDPARAMS, "Bounds must be square", "PagedGeometry::setBounds()");
	if (bounds.width() <= 0 || bounds.height() <=0)
		OGRE_EXCEPT(Ogre::Exception::ERR_INVALIDPARAMS, "Bounds must have positive width and height", "PagedGeometry::setBounds()");
...
{
	Ogre::Vector3 minimum = _convertToLocal(aab.getMinimum());
	Ogre::Vector3 maximum = _convertToLocal(aab.getMaximum());
...
void PagedGeometry::update()
{
	...
	Ogre::Vector3 camPos = _convertToLocal(sceneCam->getDerivedPosition());
	Ogre::Vector3 camSpeed;	//Speed in units-per-millisecond
	if (deltaTime == 0){
		camSpeed.x = 0;
		camSpeed.y = 0;
		camSpeed.z = 0;
...
void PagedGeometry::reloadGeometryPage(const Ogre::Vector3 &point)
{
	if (!pageLoader)
		return;
...
void PagedGeometry::reloadGeometryPages(const Ogre::Vector3 &center, Ogre::Real radius)
{
	if (!pageLoader)
		return;
...
void PagedGeometry::_addDetailLevel(GeometryPageManager *mgr, Ogre::Real maxRange, Ogre::Real transitionLength)
{
	//Calculate the near range
	Ogre::Real minRange = 0;
	if (!managerList.empty()){
...
void GeometryPageManager::update(unsigned long deltaTime, Ogre::Vector3 &camPos, Ogre::Vector3 &camSpeed, bool &enableCache, GeometryPageManager *prevManager)
{
	//-- Cache new geometry pages --

	//Cache 1 page ahead of the view ranges
	const Ogre::Real cacheDist = farTransDist + mainGeom->getPageSize();
	const Ogre::Real cacheDistSq = cacheDist * cacheDist;
	
	//First calculate the general area where the pages will be processed
	// 0,0 is the left top corner of the bounding box
	int x1 = (int)Ogre::Math::Floor(((camPos.x - cacheDist) - gridBounds.left) / mainGeom->getPageSize());
	int x2 = (int)Ogre::Math::Floor(((camPos.x + cacheDist) - gridBounds.left) / mainGeom->getPageSize());
	int z1 = (int)Ogre::Math::Floor(((camPos.z - cacheDist) - gridBounds.top) / mainGeom->getPageSize());
	int z2 = (int)Ogre::Math::Floor(((camPos.z + cacheDist) - gridBounds.top) / mainGeom->getPageSize());
	...
	for (int x = x1; x <= x2; ++x){
		for (int z = z1; z <= z2; ++z){
			...
			Ogre::Real dx = camPos.x - blk->_centerPoint.x;
			Ogre::Real dz = camPos.z - blk->_centerPoint.z;
	...
	//200 milliseconds.
	Ogre::Real speed = Ogre::Math::Sqrt(camSpeed.x * camSpeed.x + camSpeed.z * camSpeed.z);
	...
	Ogre::Real halfPageSize = mainGeom->getPageSize() * 0.5f;
	while (i1 != i2)
	{
		GeometryPage *blk = *i1;
...
void GeometryPageManager::reloadGeometryPage(const Ogre::Vector3 &point)
{
	//Determine which grid block contains the given points
	int x = (int)Ogre::Math::Floor(geomGridX * (point.x - gridBounds.left) / gridBounds.width());
	int z = (int)Ogre::Math::Floor(geomGridZ * (point.z - gridBounds.top) / gridBounds.height());
...
void GeometryPageManager::reloadGeometryPages(const Ogre::Vector3 &center, Ogre::Real radius)
{
	...
	int x1 = (int)Ogre::Math::Floor(geomGridX * (area.left - gridBounds.left) / gridBounds.width());
	int z1 = (int)Ogre::Math::Floor(geomGridZ * (area.top - gridBounds.top) / gridBounds.height());
	if (x1 < 0) x1 = 0; else if (x1 > geomGridX-1) x1 = geomGridX-1;
	if (z1 < 0) z1 = 0; else if (z1 > geomGridZ-1) z1 = geomGridZ-1;
	//...and the bottom right
	int x2 = (int)Ogre::Math::Floor(geomGridX * (area.right - gridBounds.left) / gridBounds.width());
	int z2 = (int)Ogre::Math::Floor(geomGridZ * (area.bottom - gridBounds.top) / gridBounds.height());
	...
	Ogre::Real radiusSq = radius * radius;
	for (int x = x1; x <= x2; ++x) {
		for (int z = z1; z <= z2; ++z) {
			GeometryPage *page = _getGridPage(x, z);
			if (page->_loaded){
				Ogre::Vector3 pos = page->getCenterPoint();
				Ogre::Real distX = (pos.x - center.x), distZ = (pos.z - center.z);
...
void GeometryPageManager::preloadGeometry(const TBounds & area)
{
	...
	//Determine which grid block contains the top-left corner
	int x1 = (int)Ogre::Math::Floor(geomGridX * (loadarea.left - gridBounds.left) / gridBounds.width());
	int z1 = (int)Ogre::Math::Floor(geomGridZ * (loadarea.top - gridBounds.top) / gridBounds.height());
	if (x1 < 0) x1 = 0; else if (x1 > geomGridX-1) x1 = geomGridX-1;
	if (z1 < 0) z1 = 0; else if (z1 > geomGridZ-1) z1 = geomGridZ-1;
	//...and the bottom right
	int x2 = (int)Ogre::Math::Floor(geomGridX * (loadarea.right - gridBounds.left) / gridBounds.width());
	int z2 = (int)Ogre::Math::Floor(geomGridZ * (loadarea.bottom - gridBounds.top) / gridBounds.height());
	if (x2 < 0) x2 = 0; else if (x2 > geomGridX-1) x2 = geomGridX-1;
	if (z2 < 0) z2 = 0; else if (z2 > geomGridZ-1) z2 = geomGridZ-1;
...
const Ogre::AxisAlignedBox &GeometryPage::getBoundingBox()
{
	return _trueBounds;
}
...
{
	_trueBounds = Ogre::AxisAlignedBox(0, 0, 0, 0, 0, 0);
	_trueBoundsUndefined = true;
}
}
