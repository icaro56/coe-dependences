...
TreeLoader2D::TreeLoader2D(PagedGeometry *geom, const TBounds &bounds)
{
	...
	pageGridX = (int)Ogre::Math::Ceil(gridBounds.width() / pageSize) + 1;
	pageGridZ = (int)Ogre::Math::Ceil(gridBounds.height() / pageSize) + 1;
...
void TreeLoader2D::addTree(Ogre::Entity *entity, const Ogre::Vector3 &position, Ogre::Degree yaw, Ogre::Real scale, void* userData)
{
	...
	if (pos.x < actualBounds.left-smallVal || pos.x > actualBounds.right+smallVal || pos.z < actualBounds.top-smallVal || pos.z > actualBounds.bottom+smallVal)
		OGRE_EXCEPT(Ogre::Exception::ERR_INVALIDPARAMS, "Tree position is out of bounds", "TreeLoader::addTree()");
	if (scale < minimumScale || scale > maximumScale)
		OGRE_EXCEPT(Ogre::Exception::ERR_INVALIDPARAMS, "Tree scale out of range", "TreeLoader::addTree()");
	#endif
   ...
   else
   {
	...
	int pageX = (int)Ogre::Math::Floor(xrel / pageSize); // bad perfomance float --> int
	int pageZ = (int)Ogre::Math::Floor(zrel / pageSize); // bad perfomance float --> int
	std::vector<TreeDef> &treeList = _getGridPage(pageGrid, pageX, pageZ);

	//Create the new tree
	TreeDef tree;
	tree.xPos      = static_cast<Ogre::uint16>(0xFFFF * (xrel - (pageX * pageSize)) / pageSize);
	tree.zPos      = static_cast<Ogre::uint16>(0xFFFF * (zrel - (pageZ * pageSize)) / pageSize);
   tree.rotation  = static_cast<Ogre::uint8>(Ogre::Real(0.708) * yaw.valueDegrees());                       // 0.708 ~= 255 / 360
   tree.scale     = static_cast<Ogre::uint8>(Ogre::Real(255.) * ((scale - minimumScale) / maximumScale));
	...
	geom->reloadGeometryPage(Ogre::Vector3(pos.x, 0, pos.z));
}
...
void
#endif
TreeLoader2D::deleteTrees(const Ogre::Vector3 &position, Ogre::Real radius, Ogre::Entity *type)
{
	//First convert the coordinate to PagedGeometry's local system
#ifdef PAGEDGEOMETRY_ALTERNATE_COORDSYSTEM
	Vector3 pos = geom->_convertToLocal(position);
	...
	int minPageX = (int)Ogre::Math::Floor(((x-radius) - gridBounds.left) / pageSize);
	int minPageZ = (int)Ogre::Math::Floor(((z-radius) - gridBounds.top) / pageSize);
	int maxPageX = (int)Ogre::Math::Floor(((x+radius) - gridBounds.left) / pageSize);
	int maxPageZ = (int)Ogre::Math::Floor(((z+radius) - gridBounds.top) / pageSize);
	...
	while (it != end){
		...
		for (int tileZ = minPageZ; tileZ <= maxPageZ; ++tileZ){
            ...
            {
					//Get tree distance
               Ogre::Real distX = (gridBounds.left + (tileX * pageSize) + ((Ogre::Real)treeList[i].xPos / 65535) * pageSize) - pos.x;
					Ogre::Real distZ = (gridBounds.top + (tileZ * pageSize) + ((Ogre::Real)treeList[i].zPos / 65535) * pageSize) - pos.z;
				...
				//Rebuild geometry if necessary
				if (modified){
					Ogre::Vector3 pos(gridBounds.left + ((0.5f + tileX) * pageSize), 0, gridBounds.top + ((0.5f + tileZ) * pageSize));
...
TreeLoader2D::deleteTrees(TBounds area, Ogre::Entity *type)
{
	...
	//Determine the grid blocks which might contain the requested trees
	int minPageX = (int)Ogre::Math::Floor((area.left - gridBounds.left) / pageSize);
	int minPageZ = (int)Ogre::Math::Floor((area.top - gridBounds.top) / pageSize);
	int maxPageX = (int)Ogre::Math::Floor((area.right - gridBounds.left) / pageSize);
	int maxPageZ = (int)Ogre::Math::Floor((area.bottom - gridBounds.top) / pageSize);
		...
		for (int tileZ = minPageZ; tileZ <= maxPageZ; ++tileZ){
			for (int tileX = minPageX; tileX <= maxPageX; ++tileX){
				...
				while (i < treeList.size()){
					...
					Ogre::Real posX = (gridBounds.left + (tileX * pageSize) + ((Ogre::Real)treeList[i].xPos / 65535) * pageSize);
					Ogre::Real posZ = (gridBounds.top + (tileZ * pageSize) + ((Ogre::Real)treeList[i].zPos / 65535) * pageSize);
...
void TreeLoader2D::loadPage(PageInfo &page)
{
	...
	page.xIndex -= (int)Ogre::Math::Floor(gridBounds.left / pageSize);
	page.zIndex -= (int)Ogre::Math::Floor(gridBounds.top / pageSize);
	...
	for (i = pageGridList.begin(); i != pageGridList.end(); ++i){
		...
		for (o = treeList.begin(); o != treeList.end(); ++o){
			//Get position
			Ogre::Vector3 pos;
			pos.x = page.bounds.left + ((Ogre::Real)o->xPos / 65535) * pageSize;
			pos.z = page.bounds.top + ((Ogre::Real)o->zPos / 65535) * pageSize;
			...
			//Get rotation
			Ogre::Degree angle((Ogre::Real)o->rotation * (360.0f / 255));
			Ogre::Quaternion rot(angle, Ogre::Vector3::UNIT_Y);

			//Get scale
			Ogre::Vector3 scale;
			scale.y = (Ogre::Real)o->scale * (maximumScale / 255) + minimumScale;
			scale.x = scale.y;
			scale.z = scale.y;
...
void TreeIterator2D::_readTree()
{
	...
	//Get position
	Ogre::Real boundsLeft = trees->gridBounds.left + trees->pageSize * currentX;
	Ogre::Real boundsTop = trees->gridBounds.top + trees->pageSize * currentZ;
	currentTreeDat.position.x = boundsLeft + ((Ogre::Real)treeDef.xPos / 65535) * trees->pageSize;
	currentTreeDat.position.z = boundsTop + ((Ogre::Real)treeDef.zPos / 65535) * trees->pageSize;
	...
	//Get rotation
	currentTreeDat.yaw = Ogre::Degree((Ogre::Real)treeDef.rotation * (360.0f / 255));

	//Get scale
	currentTreeDat.scale = (Ogre::Real)treeDef.scale * (trees->maximumScale / 255) + trees->minimumScale;
