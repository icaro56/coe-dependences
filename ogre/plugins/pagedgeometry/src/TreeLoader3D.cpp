...
void TreeLoader3D::addTree(Ogre::Entity *entity, const Ogre::Vector3 &position, Ogre::Degree yaw, Ogre::Real scale, void* userData)
{
	...
	int pageX = (int)Ogre::Math::Floor(xrel / pageSize);
	int pageZ = (int)Ogre::Math::Floor(zrel / pageSize);
...
void
#endif
TreeLoader3D::deleteTrees(const Ogre::Vector3 &position, Ogre::Real radius, Ogre::Entity *type)
{
	//First convert the coordinate to PagedGeometry's local system
	#ifdef PAGEDGEOMETRY_ALTERNATE_COORDSYSTEM
	Vector3 pos = geom->_convertToLocal(position);
	...
	int minPageX = (int)Ogre::Math::Floor(((pos.x-radius) - gridBounds.left) / pageSize);
	int minPageZ = (int)Ogre::Math::Floor(((pos.z-radius) - gridBounds.top) / pageSize);
	int maxPageX = (int)Ogre::Math::Floor(((pos.x+radius) - gridBounds.left) / pageSize);
	int maxPageZ = (int)Ogre::Math::Floor(((pos.z+radius) - gridBounds.top) / pageSize);
...
void TreeLoader3D::setColorMap(Ogre::TexturePtr map, MapChannel channel)
{
	if (colorMap){
		colorMap->unload();
		colorMap = NULL;
