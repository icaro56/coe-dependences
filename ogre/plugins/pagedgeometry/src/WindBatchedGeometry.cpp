...
//-----------------------------------------------------------------------------
///
WindBatchedGeometry::WindBatchedGeometry(Ogre::SceneManager *mgr, Ogre::SceneNode *rootSceneNode, const PagedGeometry *pagedGeom) : 
...
//-----------------------------------------------------------------------------
///
void WindBatchedGeometry::addEntity(Ogre::Entity *ent, const Ogre::Vector3 &position, const Ogre::Quaternion &orientation, const Ogre::Vector3 &scale, const Ogre::ColourValue &color)
{
	Ogre::MeshPtr mesh = ent->getMesh();
	if (mesh->sharedVertexData != NULL)
		OGRE_EXCEPT(Ogre::Exception::ERR_INVALIDPARAMS, "Shared vertex data not allowed", "BatchedGeometry::addEntity()");
...
//-----------------------------------------------------------------------------
/// Constructor
WindBatchedGeometry::WindSubBatch::WindSubBatch(WindBatchedGeometry *parent, Ogre::SubEntity *ent) : 
...
void WindBatchedGeometry::WindSubBatch::build()
{
	...
	//Misc. setup
	const Ogre::Vector3 &batchCenter = static_cast < WindBatchedGeometry* > (m_pParentGeom)->m_vecCenter;

	Ogre::HardwareIndexBuffer::IndexType srcIndexType = m_pSubMesh->indexData->indexBuffer->getType();
	Ogre::HardwareIndexBuffer::IndexType destIndexType = 
	...
	Ogre::uint32 *indexBuffer32 = 0;
	Ogre::uint16 *indexBuffer16 = 0;
   ...
   {
      const Ogre::VertexDeclaration::VertexElementList &vlist = vertDecl->getElements();
      Ogre::VertexDeclaration::VertexElementList::const_iterator it = vlist.begin(), iend = vlist.end();
      while (it != iend)
      {
         if ((*it).getSemantic() == Ogre::VES_TEXTURE_COORDINATES)
            ++texCoordCount;
         ++it;
      }
   }
   ...
   vertDecl->addElement(numVertBuffs - 1, vertDecl->getVertexSize(0), Ogre::VET_FLOAT4 , Ogre::VES_TEXTURE_COORDINATES, texCoordCount);
   vertDecl->addElement(numVertBuffs - 1, vertDecl->getVertexSize(0), Ogre::VET_FLOAT4 , Ogre::VES_TEXTURE_COORDINATES, texCoordCount+1);

   //Allocate & lock the vertex buffers
   std::vector<Ogre::uchar*> vertexBuffers(numVertBuffs);
   std::vector<Ogre::VertexDeclaration::VertexElementList> vertexBufferElements(numVertBuffs);
	...
	for (Ogre::ushort i = 0; i < numVertBuffs; ++i)
	{
		...
		vertexBuffers[i] = static_cast<Ogre::uchar*>(buffer->lock(Ogre::HardwareBuffer::HBL_DISCARD));
   ...
   for (size_t iMesh = 0, meshCnt = m_queueMesh.size(); iMesh < meshCnt; ++iMesh)
   {
      ...
      const Ogre::Vector3 &scale = queuedMesh.scale;
		...
		const Ogre::IndexData *sourceIndexData = queuedMesh.subMesh->indexData;
		const Ogre::VertexData *sourceVertexData = queuedMesh.subMesh->vertexData;
		Ogre::Entity * ent = static_cast<Ogre::Entity*>(queuedMesh.userData);
      ...
      Ogre::Real invMaxHeight = Ogre::Real(1.) / ent->getBoundingBox().getMaximum().y;
		...
		Ogre::VertexBufferBinding *destBinds = m_pVertexData->vertexBufferBinding;
      ...
      Ogre::Vector3 v3AddBatchCenter = queuedMesh.position - batchCenter;
		...
		for (Ogre::ushort i = 0; i < destBinds->getBufferCount(); ++i)
		{
			if (i < sourceBinds->getBufferCount())
         {
				...
				const Ogre::HardwareVertexBufferSharedPtr &sourceBuffer = sourceBinds->getBuffer(i);
            ...
            size_t destVertexSize   = vertDecl->getVertexSize(i);
				...
				Ogre::uchar *destBase = vertexBuffers[i];

            const Ogre::VertexDeclaration::VertexElementList &elems = vertexBufferElements[i];
            ...
            float *sourcePtr = 0, *destPtr = 0;
				...
				{
               Ogre::VertexDeclaration::VertexElementList::const_iterator itElement = iBegin;
               // Iterate over vertex elements
					for ( ; itElement != iEnd; ++itElement)
					{
						const Ogre::VertexElement &elem = *itElement;
                  ...
                  case Ogre::VES_TANGENT:
                     ...
                     {
                        Ogre::uint32 tmpColor = *(reinterpret_cast<Ogre::uint32*>(sourcePtr));
                        ...
                        Ogre::uint8 tmpA = static_cast<Ogre::uint8>(((tmpColor >> 24) & 0xFF) * queuedMesh.color.a);

                        tmpColor = tmpR | (tmpG << 8) | (tmpB << 16) | (tmpA << 24);
                        *(reinterpret_cast<Ogre::uint32*>(destPtr)) = tmpColor;
                     }
							break;
         ...
         else
         {
				...
				//Get the locked output buffer
				Ogre::uint32 *startPtr = (Ogre::uint32*)vertexBuffers[vertBinding->getBufferCount()-1];
				...
				while (startPtr < endPtr)
            {
				...
				vertexBuffers[vertBinding->getBufferCount()-1] += (sizeof(Ogre::uint32) * sourceVertexData->vertexCount);
         }
		}
