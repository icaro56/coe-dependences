...
#include "OgreMaxPrerequisites.h"
...
{   
     ...
     */
    class OGREMAX_EXPORT OgreMaxScene : public Ogre::RenderTargetListener
    {
    public:
        OgreMaxScene();
         ...
         * @param fileName [in] - The name of the file to load. This file name can contain a directory path
        ...
        /** Gets the collection of all model templates, each of which is keyed by name */
        const ModelMap& GetModelMap() const;
		const std::map<Ogre::String, Ogre::MovableObject*>& GetLoadedObjects() const;

        /** Gets the model template with the specified name, loading it if it isn't already loaded */
        OgreMaxModel* GetModel(const Ogre::String& name);
        ...
        class OGREMAX_EXPORT ResourceLocation
        {    
        public:
            ResourceLocation()
            {
    ...
    class OGREMAX_EXPORT OgreMaxSceneCallback
    {   
    public:
