...
{
    //Parse the directory and file base name from the input file name
    String directory, fileBaseName;
    StringUtil::splitFilename(fileName, fileBaseName, directory);
    if (!directory.empty())
    {
...
const std::map<Ogre::String, Ogre::MovableObject*>& OgreMaxScene::GetLoadedObjects() const
{
    return this->loadedObjects;
}
OgreMaxModel* OgreMaxScene::GetModel(const String& name)
{
    //Look up item and return it if found
    ModelMap::iterator item = this->modelMap.find(name);
    ...
    try
    {
        model->Load(name, this->defaultResourceGroupName);
    }
    catch (...)
    {
        delete model;
...
void OgreMaxScene::CommitResourceLocations()
{
        ...
        ++resourceLocationIterator)
    {
        if (!resourceLocationIterator->initialized)
        {
            ...
            //TODO: erro aqui
            //resourceLocationIterator->initialized = true;
        }
    }
...
void OgreMaxScene::FinishLoadingRenderTextures()
{
    static const Quaternion CUBE_FACE_CAMERA_ORIENTATIONS[] =
        {
        Quaternion(0.707107f, 0.0f, -0.707107f, 0.0f),
        Quaternion(-0.707107f, 0.0f, -0.707107f, 0.0f),
        Quaternion(0.707107f, 0.707107f, 0.0f, 0.0f),
        Quaternion(0.707107f, -0.707107f, 0.0f, 0.0f),
...
void OgreMaxScene::LoadNode(const TiXmlElement* objectElement, SceneNode* parentNode)
{
    ...
    //Get/load the model
    String modelName = OgreMaxUtilities::GetStringAttribute(objectElement, "modelName");
    OgreMaxModel* model = modelName.empty() ? 0 : GetModel(modelName);
