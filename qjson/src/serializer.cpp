...
QByteArray Serializer::serialize( const QVariant &v )
{
  ...
  if ( ! v.isValid() ) { // invalid or null?
	...
	int precision = 6; // 6 is the default precision value to QByteArray::number

      // Convert to string using as many digits of precision as
     // std::numeric_limits<...>::digits10 specifies
      ...
      str = QByteArray::number( v.toDouble(), 'g', precision ); // 'g' is the default parameter value
